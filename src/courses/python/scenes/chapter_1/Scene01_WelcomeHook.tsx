...
export const Scene01_WelcomeHook: React.FC = () => (
  <div className="h-full w-full flex flex-col items-center justify-center px-8 py-6 overflow-hidden relative">
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="absolute bottom-1/4 right-1/3 w-[350px] h-[350px] rounded-full bg-sky-400/6 filter blur-[120px]" />
    </div>
    ...
    <div className="relative z-10 flex flex-col items-center gap-6 w-full max-w-2xl">
      ...
      >
        <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-slate-800 leading-[1.05]">One language.</h1>
        <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-amber-500 leading-[1.05]">Infinite possibilities.</h1>
      </motion.div>
      ...
      <motion.p
        ...
        className="text-slate-500 text-sm lg:text-base font-medium text-center"
      >
      ...
      <div className="flex flex-col gap-2.5 w-full">
        {LINES.map((item, idx) => (
          <motion.div
            ...
            transition={{ delay: 0.8 + idx * 0.2, duration: 0.5, ease: 'easeOut' }}
            className="flex items-center justify-between gap-6 bg-slate-900 rounded-2xl px-5 py-3 border border-slate-800"
          >
            <span className={`font-mono text-sm lg:text-base font-bold ${item.color}`}>{item.code}</span>
            <span className="text-slate-500 text-xs font-medium shrink-0 hidden sm:block">→ {item.label}</span>
          </motion.div>
        ))}
      </div>
